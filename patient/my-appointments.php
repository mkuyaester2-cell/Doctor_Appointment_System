    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <div class="flex space-x-2 bg-white p-1 rounded-xl shadow-sm border border-slate-200">
                <a href="?filter=all" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors <?php echo $filter === 'all' ? 'bg-primary-100 text-primary-700' : 'text-slate-600 hover:bg-slate-50'; ?>">All</a>
                <a href="?filter=upcoming" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors <?php echo $filter === 'upcoming' ? 'bg-primary-100 text-primary-700' : 'text-slate-600 hover:bg-slate-50'; ?>">Upcoming</a>
                <a href="?filter=past" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors <?php echo $filter === 'past' ? 'bg-primary-100 text-primary-700' : 'text-slate-600 hover:bg-slate-50'; ?>">Past</a>
            </div>
        </div>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between gap-6 transition-all hover:shadow-md">
                    <div class="flex gap-4">
                        <div class="flex flex-col items-center justify-center w-16 h-16 bg-slate-50 rounded-xl border border-slate-100 text-slate-700">
                        ...
                        <div>
                            <div class="flex items-center gap-3 mb-1">
                                ...
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-bold uppercase border <?php echo $statusClass; ?>">
                                    <?php echo $apt['status']; ?>
                                </span>
                            </div>
                    ...
                    <div class="flex items-center gap-3 self-start md:self-center">
                         <?php if ($apt['status'] === 'pending' || $apt['status'] === 'confirmed'): ?>
                             <a href="cancel-appointment.php?id=<?php echo $apt['id']; ?>" class="px-4 py-2 text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">Cancel</a>
                         <?php endif; ?>
                         <a href="appointment-confirmation.php?id=<?php echo $apt['id']; ?>" class="px-4 py-2 text-sm font-medium text-slate-600 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">Details</a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
